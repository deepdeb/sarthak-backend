...
exports.createCustomer = async (data) => {
    try {
        ...
        let insert_customer_sql = "INSERT INTO customer (sbu_id, sales_person_id, customer_create_date, customer, segment_id, subsegment_id, subsubsegment_id, subsubsubsegment_id, name, designation, department, mobile, alt_mobile, email, alt_email, product_category_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        const [insert_customer_resp] = await writePool.query(insert_customer_sql, [data.sbu_id, data.sales_person_id, data.customer_create_date, data.customer, data.segment_id, data.subsegment_id, data.subsubsegment_id, data.subsubsubsegment_id, data.name, data.designation, data.department, data.mobile, data.alt_mobile, data.email, data.alt_email, data.product_category_id]);
...
exports.editCustomer = async (data) => {
    try {
        ...
        let update_customer_sql = "UPDATE customer SET sbu_id = ?, sales_person_id = ?, customer_create_date = ?, customer = ?, segment_id = ?, subsegment_id = ?, subsubsegment_id = ?, subsubsubsegment_id = ?, name = ?, designation = ?, department = ?, mobile = ?, alt_mobile = ?, email = ?, alt_email = ?, product_category_id = ? WHERE customer_id = ?"
        const [update_customer_resp] = await writePool.query(update_customer_sql, [data.sbu_id, data.sales_person_id, data.customer_create_date, data.customer, data.segment_id, data.subsegment_id, data.subsubsegment_id, data.subsubsubsegment_id, data.name, data.designation, data.department, data.mobile, data.alt_mobile, data.email, data.alt_email, data.product_category_id, data.customer_id]);
