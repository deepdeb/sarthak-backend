...
exports.createCustomer = async (data) => {
    try {
        ...
        let insert_customer_sql = "INSERT INTO customer (date, sbu_id, sales_person_id, mentor_id, customer, segment_id, subsegment_id, name, designation, department, mobile, email, product_category_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
        const [insert_customer_resp] = await writePool.query(insert_customer_sql, [data.date, data.sbu_id, data.sales_person_id, data.mentor_id, data.customer, data.segment_id, data.subsegment_id, data.name, data.designation, data.department, data.mobile, data.email, data.product_category_id]);
