...
exports.createEnquiry = async (data) => {
    try {
        ...
        const [resp] = await writePool.query(insert_enquiry_sql, [data.sbu_id, data.sales_person_id, data.customer_id, data.enquiry_date, data.enquiry_source_id, data.principal_house, offer_date, data.basic_value, data.tentative_finalization_month, data.tentative_finalization_year, data.status_initial, data.remarks_initial, data.support_initial]);
        if(resp) {
...
exports.editEnquiry = async (data) => {
    try {
        ...
        const [update_resp] = await writePool.query(update_sql, [data.sbu_id, data.sales_person_id, data.customer_id, data.enquiry_date, data.enquiry_source_id, data.principal_house, offer_date, data.basic_value, data.tentative_finalization_month, data.tentative_finalization_year, data.status_initial, data.remarks_initial, data.support_initial, data.enquiry_id]);
