...
exports.createOrder = async (data) => {
    try {
        ...
        const [resp] = await writePool.query(insert_order_sql, [data.sales_person_id, data.sbu_id, data.customer_id, data.po_number, data.po_date, data.po_type_id, data.product, data.supply_description, data.brand, data.sitc_description, data.cable_assembly, data.panel, data.welding_receptable, data.clamps, data.hsa_box, data.others, data.basic_po_value, data.total_po_value, data.scheduled_completion_date, data.actual_completion_date, data.purchase_order_file, data.completion_file, data.credential_file]);
        if(resp) {
