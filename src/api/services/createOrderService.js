...
exports.editOrder = async (data) => {
    try {
        let edit_order_sql = "UPDATE orders SET po_type_id = ?, product = ?, supply_description = ?, brand = ?, sitc_description = ?, cable_assembly = ?, panel = ?, welding_receptable = ?, clamps = ?, hsa_box = ?, others = ?, basic_po_value = ?, total_po_value = ?, scheduled_completion_date = ?, actual_completion_date = ?, purchase_order_file = ?, completion_file = ?, credential_file = ? WHERE order_id = ?"
        const [resp] = await writePool.query(edit_order_sql, [data.po_type_id, data.product, data.supply_description, data.brand, data.sitc_description, data.cable_assembly, data.panel, data.welding_receptable, data.clamps, data.hsa_box, data.others, data.basic_po_value, data.total_po_value, data.scheduled_completion_date, data.actual_completion_date, data.purchase_order_file, data.completion_file, data.credential_file, data.order_id])
        if(resp) {
