...
exports.getOrderReport = async (data) => {
    try {
        ...
        let sql = "SELECT o.customer_id, c.customer, DATE_FORMAT(o.po_date, '%d-%m-%Y') as po_date, o.po_number, o.po_type_id, p.po_type_name, o.product, o.supply_description, o.brand, o.sitc_description, o.cable_assembly, o.panel, o.welding_receptable, o.hsa_box, o.others, o.po_subtype_id, post.po_subtype_name, o.basic_po_value, o.total_po_value, DATE_FORMAT(o.scheduled_completion_date, '%d-%m-%Y') as scheduled_completion_date, DATE_FORMAT(o.actual_completion_date, '%d-%m-%Y') as actual_completion_date FROM orders o JOIN customer c ON o.customer_id = c.customer_id JOIN po_type p ON p.po_type_id = o.po_type_id LEFT JOIN po_subtype post ON post.po_subtype_id = o.po_subtype_id" + searchCondition + searchCondition2 + ""
